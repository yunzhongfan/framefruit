...
function doSave(){
	$.messager.confirm('修改规则指标', '确定要修改该规则指标？', function(data) {
		if (!data) {
			return false;
		}else{
...
function updateRating(){
	$.ajax({
		url: ctx+"/rule/updateRatingind",
		type:"post",
		data:$("#addRatingindForm").serialize(),
		success:function(result){
			 updateRulePackageInd();
			},
	        error:function(){
	        	$.messager.alert("错误","修改指标出错，请联系管理员！","error");
	        }
...
function updateRulePackageInd(){
		...
		$.ajax({
    	        url : ctx + "/rule/updateRulePackageInd",
    	        data : {
    	        	"package_ind_id" : $("#package_ind_id").val(),
    	        	"rule_content" : calData
    	        },
    	        type : 'POST',
    	        success : function(r) {
    	        	 $.messager.alert('规则指标',"已成功修改该规则指标！",'info',function(data){
    	        ...
    	        error:function(){
    	        	$.messager.alert('错误','修改规则内容出错，请联系管理员！','error');
    	        }
    	    });
}
